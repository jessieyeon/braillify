...
async function fetchBraille(input: string): Promise<string> {
  ...
  const data = (await res.json()) as { ascii: string; braille: string; count: number };
  return data.braille;
}
...
async function processFile(filePath: string): Promise<{ total: number; fetched: number; skipped: number; errors: number }> {
...
async function main() {
  ...
  for (const dir of dirs) {
    ...
    try {
      ...
      for (const file of jsonFiles) {
        ...
        console.log(`✓ ${stats.fetched} fetched, ${stats.skipped} skipped, ${stats.errors} errors (${stats.total} total)`);
  ...
  console.log(`Fetched: ${grandFetched} | Skipped: ${grandSkipped} | Errors: ${grandErrors}`);
