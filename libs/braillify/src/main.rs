...
mod tests {
    ...
    fn get_built_binary() -> &'static escargot::CargoRun {
        BUILT_BINARY.get_or_init(|| {
            ...
            for attempt in 1..=3 {
                match escargot::CargoBuild::new().bin("braillify").current_release().current_target().run() {
    ...
    fn test_braillify_help() {
        let mut cmd = get_built_binary().command();
        cmd.arg("--help");
        cmd.assert().success().stdout(predicate::str::contains("한국어 점자 변환 CLI"));
    }
    ...
    fn test_braillify_version() {
        let mut cmd = get_built_binary().command();
        cmd.arg("--version");
        cmd.assert().success().stdout(predicate::str::contains("braillify"));
    }
    ...
    fn test_braillify_no_args() {
        ...
        assert!(output.status.success());
        assert!(!output.stdout.is_empty());
        cmd.assert().success().stdout(predicate::str::contains("braillify REPL"));
    }
