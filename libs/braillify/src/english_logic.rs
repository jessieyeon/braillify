...
pub(crate) fn next_ascii_letter_or_digit(word_chars: &[char], index: usize, remaining_words: &[&str]) -> bool {
    let mut j = index + 1;
...
pub(crate) fn should_render_symbol_as_english(english_indicator: bool, is_english: bool, parenthesis_stack: &[bool], symbol: char, word_chars: &[char], index: usize, remaining_words: &[&str]) -> bool {
    if !english_indicator {
        return false;
    }

    let prev_char = if index > 0 { Some(word_chars[index - 1]) } else { None };
    let next_char = if index + 1 < word_chars.len() { Some(word_chars[index + 1]) } else { remaining_words.first().and_then(|w| w.chars().next()) };
    ...
    match symbol {
        ...
        '/' | '@' | '#' | '.' | '_' | ':' | '-' => {
            ...
            (prev_ascii && next_ascii) || (symbol == '/' && prev_char == Some('/') && next_ascii) || (symbol == '/' && next_char == Some('/') && prev_ascii)
        }
        _ => false,
    }
}

pub(crate) fn should_keep_english_mode_for_symbol(symbol: char, word_chars: &[char], index: usize, remaining_words: &[&str]) -> bool {
...
mod tests {
    ...
    fn should_render_symbol_as_english_for_parentheses() {
        ...
        assert!(should_render_symbol_as_english(true, false, &[], '(', &opener, 0, &[]));

        let korean_before: Vec<char> = "가(".chars().collect();
        assert!(!should_render_symbol_as_english(true, false, &[], '(', &korean_before, 1, &["A"]));

        assert!(!should_render_symbol_as_english(false, false, &[], '(', &opener, 0, &[]));
    }

    #[test]
    fn should_render_symbol_as_english_for_closing_parenthesis() {
        let closer: Vec<char> = ")".chars().collect();
        assert!(should_render_symbol_as_english(true, true, &[true], ')', &closer, 0, &[]));

        assert!(!should_render_symbol_as_english(true, true, &[false], ')', &closer, 0, &[]));
    }

    #[test]
    fn should_render_symbol_as_english_for_comma_requires_ascii_neighbors() {
        let word: Vec<char> = "A,B".chars().collect();
        assert!(should_render_symbol_as_english(true, true, &[], ',', &word, 1, &[]));

        let no_english_context: Vec<char> = "A,B".chars().collect();
        assert!(!should_render_symbol_as_english(true, false, &[], ',', &no_english_context, 1, &[]));

        let korean_neighbor: Vec<char> = "가,B".chars().collect();
        assert!(!should_render_symbol_as_english(true, true, &[], ',', &korean_neighbor, 1, &[]));
    }
}
