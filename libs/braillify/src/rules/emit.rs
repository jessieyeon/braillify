...
fn is_math_operator_space_suppression<'a>(tokens: &'a [Token<'a>], space_idx: usize) -> bool {
    ...
    fn token_is_relation_operator_word(token: Option<&Token<'_>>) -> bool {
        match token {
            Some(Token::Word(w)) => w.chars.len() <= 2 && w.chars.iter().all(|c| matches!(*c, '=' | '<' | '>' | '\u{2260}' | '\u{2264}' | '\u{2265}')),
    ...
    if space_idx + 1 < tokens.len() && token_is_relation_operator_word(tokens.get(space_idx + 1)) && space_idx > 0 && token_is_math_word(tokens.get(space_idx - 1)) {
        return true;
    }
    // 케이스 2: Space 이전이 관계 연산자 Word, 다음이 math Word/PreEncoded.
    if space_idx > 0 && token_is_relation_operator_word(tokens.get(space_idx - 1)) && space_idx + 1 < tokens.len() && token_is_math_word(tokens.get(space_idx + 1)) {
        return true;
    }
    false
}
...
pub fn emit(ir: &mut DocumentIR, char_engine: &mut RuleEngine) -> Result<Vec<u8>, String> {
    ...
    let word_texts = if ir.tokens.len() > 1 { collect_word_texts(&ir.tokens) } else { Vec::new() };
    let mut word_index = 0usize;

    for (idx, token) in ir.tokens.iter().enumerate() {
        match token {
            Token::Word(word) => {
                let context = if word_texts.is_empty() { WordContext { prev_word: "", remaining_words: &[] } } else { word_context(&word_texts, word_index) };
                emit_word(word, idx, &mut ir.state, char_engine, &ir.tokens, context, &mut result)?;
                word_index += 1;
            }
            ...
            Token::Fraction(frac) => {
                if let Some(ref w) = frac.whole {
                    result.extend(fraction::encode_mixed_fraction(w, &frac.numerator, &frac.denominator)?);
                } else {
                    result.extend(fraction::encode_fraction(&frac.numerator, &frac.denominator)?);
                }
...
fn word_context<'a>(word_texts: &'a [&'a str], word_index: usize) -> WordContext<'a> {
    let prev_word = word_index.checked_sub(1).map_or("", |prev_index| word_texts[prev_index]);
    let remaining_words = &word_texts[word_index + 1..];

    WordContext { prev_word, remaining_words }
}
...
fn apply_core_encoding_rules(engine: &mut RuleEngine, char_type: &CharType, word_chars: &[char], index: usize, is_all_uppercase: bool, has_korean_char: bool, ascii_starts_at_beginning: bool, state: &mut EncoderState, skip_count: &mut usize, remaining_words: &[&str], prev_word: &str, result: &mut Vec<u8>) -> Result<crate::rules::traits::RuleResult, String> {
    let mut ctx = RuleContext { word_chars, index, char_type, prev_word, remaining_words, has_korean_char, is_all_uppercase, ascii_starts_at_beginning, skip_count, state, result };
    engine.apply_phase(Phase::CoreEncoding, &mut ctx)
}

#[allow(clippy::too_many_arguments)]
fn apply_inter_character_rules(engine: &mut RuleEngine, char_type: &CharType, word_chars: &[char], index: usize, is_all_uppercase: bool, has_korean_char: bool, ascii_starts_at_beginning: bool, state: &mut EncoderState, skip_count: &mut usize, remaining_words: &[&str], prev_word: &str, result: &mut Vec<u8>) -> Result<(), String> {
    let mut ctx = RuleContext { word_chars, index, char_type, prev_word, remaining_words, has_korean_char, is_all_uppercase, ascii_starts_at_beginning, skip_count, state, result };
...
fn emit_word(word: &WordToken, token_index: usize, state: &mut EncoderState, char_engine: &mut RuleEngine, all_tokens: &[Token], context: WordContext<'_>, result: &mut Vec<u8>) -> Result<(), String> {
    ...
    if word_len > 0 {
        ...
        if state.english_indicator && !state.is_english && has_ascii_alphabetic && word_chars[0].is_ascii_alphabetic() {
        ...
        for (i, c) in word_chars.iter().enumerate() {
            ...
            if state.english_indicator && state.is_english {
                match &char_type {
                    ...
                    CharType::Symbol(sym) => {
                        ...
                        let prev_wrap_eng_continuation = i == 0 && prev_is_hangul_wrap_end && matches!(*sym, '.' | '/' | '@' | '#' | '_' | ':' | '-') && english_logic::next_ascii_letter_or_digit(word_chars, i, remaining_words);

                        // 단어 끝의 영어 모드 유지 가능 기호(. , : ;) 직후 한글표(⠸⠷)가
                        // 이어지면, 그 기호도 영어 컨텍스트의 연속으로 본다 (제39항 wrap
                        // 직전). 예) "(Korean:" 끝의 ':'은 다음 wrap된 한글에 이어지므로
                        // 영어 점자(⠒)로 처리.
                        let next_wrap_eng_continuation = i == word_chars.len() - 1 && next_is_hangul_wrap && matches!(*sym, '.' | ',' | ':' | ';');

                        if prev_wrap_eng_continuation || next_wrap_eng_continuation || english_logic::should_render_symbol_as_english(state.english_indicator, state.is_english, &state.parenthesis_stack, *sym, word_chars, i, remaining_words) || english_logic::should_keep_english_mode_for_symbol(*sym, word_chars, i, remaining_words) {
                        } else if english_logic::should_force_terminator_before_symbol(*sym) || !english_logic::should_skip_terminator_for_symbol(*sym) {
                            result.push(50);
                            exit_english(state, false);
                        } else {
            ...
            apply_core_encoding_rules(char_engine, &char_type, word_chars, i, is_all_uppercase, has_korean_char, ascii_starts_at_beginning, state, &mut skip_count, remaining_words, prev_word, result)?;
            ...
            if let CharType::Korean(ref korean) = char_type
                && i < word_len - 1
            {
                let recon_type = CharType::Korean(KoreanChar { cho: korean.cho, jung: korean.jung, jong: korean.jong });
                state.is_number = is_number;
                state.is_big_english = is_big_english;
                apply_inter_character_rules(char_engine, &recon_type, word_chars, i, is_all_uppercase, has_korean_char, ascii_starts_at_beginning, state, &mut skip_count, remaining_words, prev_word, result)?;
                is_number = state.is_number;
                is_big_english = state.is_big_english;
            }
    ...
    if state.english_indicator && state.is_english && next_is_hangul_wrap {
        ...
        if remaining_words.is_empty() {
            ...
            let ascii_letters = next_word.chars().filter(|c| c.is_ascii_alphabetic()).collect::<Vec<_>>();
            let has_invalid_symbol = next_word.chars().any(|ch| !(ch.is_ascii_alphabetic() || english_logic::is_english_symbol(ch) || crate::symbol_shortcut::is_symbol_char(ch) || crate::utils::is_korean_char(ch)));
            let is_single_letter_word = ascii_letters.len() == 1 && !next_word.chars().any(|ch| ch.is_ascii_digit()) && !has_invalid_symbol;

            if is_single_letter_word && english_logic::requires_single_letter_continuation(ascii_letters[0]) {
                ...
                if let Ok(next_type) = CharType::new(next_char) {
                    match next_type {
                        ...
                        CharType::Symbol(sym) => {
                            if state.english_indicator && state.is_english && english_logic::is_english_symbol(sym) {
...
mod tests {
    ...
    fn english_indicator(text: &str) -> bool {
        text.split(' ').filter(|word| !word.is_empty()).any(|word| word.chars().any(utils::is_korean_char))
    }
    ...
    fn make_char_engine() -> RuleEngine {
        ...
        engine.register(Box::new(crate::rules::korean::rule_english_symbol::RuleEnglishSymbol));
    ...
    fn make_token_engine() -> crate::rules::token_engine::TokenRuleEngine {
        ...
        engine.register(Box::new(crate::rules::token_rules::quote_attachment::QuoteAttachmentRule));
    ...
    fn assert_round_trip(text: &str) {
        ...
        token_engine.apply_all(&mut ir.tokens, &mut ir.state).unwrap();
        ir.state = state_before_token_rules;
        let emitted = emit(&mut ir, &mut engine).unwrap();
        let expected = encode(text).unwrap();
        assert_eq!(emitted, expected, "round-trip mismatch for {:?}\n  emit:   {:?}\n  encode: {:?}", text, emitted, expected);
    }
    ...
    fn mode_events_emit_expected_bytes() {
        let mut ir = DocumentIR { tokens: vec![Token::Mode(ModeEvent::EnterEnglish), Token::Mode(ModeEvent::EnterEnglishContinue), Token::Mode(ModeEvent::CapsWord), Token::Mode(ModeEvent::CapsPassageStart), Token::Mode(ModeEvent::CapsPassageEnd), Token::Mode(ModeEvent::Grade1Indicator)], state: EncoderState::new(false) };
        let mut engine = make_char_engine();
    ...
    fn fraction_token_encodes() {
        let mut ir = DocumentIR { tokens: vec![Token::Fraction(super::super::token::FractionToken { whole: None, numerator: "1".to_string(), denominator: "2".to_string() }), Token::Space(SpaceKind::Regular), Token::Fraction(super::super::token::FractionToken { whole: Some("3".to_string()), numerator: "1".to_string(), denominator: "4".to_string() })], state: EncoderState::new(false) };
        let mut engine = make_char_engine();
        let out = emit(&mut ir, &mut engine).unwrap();
    ...
    fn extract_context_uses_prev_and_remaining_words() {
        ...
        let tokens = words
            ...
            .map(|w| {
                ...
                Token::Word(WordToken { text: Cow::Borrowed(w), chars: chars.clone(), meta: super::super::token::WordMeta::from_chars(&chars) })
            })
            .collect::<Vec<_>>();
