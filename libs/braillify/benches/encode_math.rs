...
fn bench_math_lines(c: &mut Criterion) {
    ...
    let expressions: Vec<&str> = corpus.lines().filter(|line| !line.trim().is_empty()).collect();
    ...
    for (index, expression) in expressions.iter().enumerate() {
        ...
        group.bench_with_input(BenchmarkId::from_parameter(label), expression, |b, expression| {
...
fn bench_math_concat(c: &mut Criterion) {
    let corpus = math_corpus();
    let concat = corpus.lines().filter(|line| !line.trim().is_empty()).collect::<Vec<_>>().join(" ");
